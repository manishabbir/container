...
DECLARE
  ...
  v_expenses_abroad NUMERIC := 0;
  v_foreign_currency TEXT; v_damages NUMERIC := 0;
  v_discounts NUMERIC := 0; v_commissions NUMERIC := 0;
  v_exchange_gl NUMERIC := 0; v_invested NUMERIC := 0; v_net_profit NUMERIC := 0;
BEGIN
  ...
  VALUES (p_container_id,v_total_sales,v_total_returns,v_net_sales,v_tax_collected,
    ...
    v_expenses_home,v_expenses_abroad,
...
BEGIN
  ...
  FROM transactions WHERE container_id=p_container_id AND status='approved'
    AND transaction_type IN ('fund_transfer','fund_receipt');
  SELECT v_balance_home - COALESCE(SUM(amount_home),0) INTO v_balance_home
...
DECLARE
  ...
  v_total_qty INTEGER:=0; v_total_cost NUMERIC:=0;
  v_alloc NUMERIC; v_sum NUMERIC:=0; v_rem NUMERIC:=0;
...
DECLARE
  v_cogs NUMERIC; v_gross NUMERIC; v_net NUMERIC;
...
BEGIN
  ...
  expected:=2224000; actual:=v_cogs; difference:=ABS(expected-actual); passed:=difference<=1; status:=CASE WHEN passed THEN 'PASS' ELSE 'FAIL' END; RETURN NEXT;
  ...
  expected:=436000; actual:=v_net; difference:=ABS(expected-actual); passed:=difference<=1; status:=CASE WHEN passed THEN 'PASS' ELSE 'FAIL' END; RETURN NEXT;
  ...
  expected:=28; actual:=v_qty; difference:=ABS(expected-actual); passed:=difference=0; status:=CASE WHEN passed THEN 'PASS' ELSE 'FAIL' END; RETURN NEXT;
  ...
  expected:=41700; actual:=v_damages; difference:=ABS(expected-actual); passed:=difference<=1; status:=CASE WHEN passed THEN 'PASS' ELSE 'FAIL' END; RETURN NEXT;
  ...
  expected:=2800000; actual:=v_balance; passed:=v_balance>0; status:=CASE WHEN passed THEN 'PASS' ELSE 'FAIL' END; RETURN NEXT;
END;
$$ LANGUAGE plpgsql;