...
-- ==========================================
-- AUTO-RECALCULATE ON SALE APPROVAL
-- ==========================================
...
BEGIN
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    IF NEW.is_return THEN
      ...
      SET quantity_sold = GREATEST(quantity_sold - NEW.quantity_sold, 0),
          quantity_returned = quantity_returned + NEW.quantity_sold,
          updated_at = NOW()
...
BEGIN
  IF TG_OP = 'UPDATE' THEN
    ...
    VALUES (auth.uid(), 'update_' || TG_TABLE_NAME, TG_TABLE_NAME, NEW.id,
            row_to_json(OLD)::jsonb, row_to_json(NEW)::jsonb);
    RETURN NEW;
  ELSIF TG_OP = 'DELETE' THEN
    INSERT INTO audit_logs (user_id, action, entity_type, entity_id, old_values)
    VALUES (auth.uid(), 'delete_' || TG_TABLE_NAME, TG_TABLE_NAME, OLD.id,
...
BEGIN
  IF TG_OP = 'INSERT' AND NEW.status = 'pending' THEN
    ...
    PERFORM create_notification(
      ...
      'approval_request', 'New Transaction Pending Approval',
      v_from_name || ' sent ' || NEW.amount_home || ' PKR. Review needed.',
      '/transactions/' || NEW.id, NEW.id, NULL);
    IF NEW.to_user_id IS NOT NULL THEN
      PERFORM create_notification(NEW.to_user_id, 'approval_request',
